    ...
    <div className='my-3'>
        <div className='card mx-4'>
            ...
            <div className='card-body'>
                <h5 className='card-title' style={{minHeight: "48px"}}>{title != null && title.length > 60 ? title.slice(0,60) + '...' : title}</h5>
                <p className='card-text' style={{minHeight: "72px"}}>{description != null && description.length > 125 ? description.slice(0,125) + '...' : description}</p>
                <p className='card-text' style={{minHeight: "48px"}}><small className='text-muted'>Published on : {new Date(date).toGMTString()}</small></p>
